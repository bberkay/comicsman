...
UZANTILAR = set(['png','jpg','jpeg','rar','zip','cbr','cbz'])
...
def myprofile(id):
    ...
    if result > 0 and result_second > 0:
        ...
        dosya = user["imgname"]
        return render_template("profile.html",users = user,comics = comic,dosya = dosya,main_id = session["id"],id = id)
    elif result > 0:
        user = cursor.fetchone()
        dosya = user["imgname"]
        return render_template("profile.html",users = user,dosya = dosya,main_id = session["id"],id = id)
    else:
